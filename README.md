...
- **Web UI**:深色仪表盘,服务器概览列表 + 单机详情页(实时图表、磁盘/网络/进程表),登录页鉴权
...
脚本会自动:安装 Go(如缺)→ 编译 → 签发/复用证书 → 生成随机密钥 → 配置 systemd → 启动。完成后会打印:

```
访问地址:     https://1.2.3.4:8553/?token=a1b2c3...
...
证书优先级:`CERT`/`KEY` 显式传入 > `DOMAIN` 自动申请 > 自签。
...
# Dashboard 是正式证书(有域名)
sudo SERVER=监控.example.com:8553 TOKEN=<上面打印的密钥> NAME=web-01 TLS=1 ./scripts/install-agent.sh

# Dashboard 是自签证书(加 INSECURE=1,连接仍加密)
sudo SERVER=1.2.3.4:8553 TOKEN=<密钥> NAME=web-01 TLS=1 INSECURE=1 ./scripts/install-agent.sh
```
...
# 一次性交叉编译所有平台(linux/darwin/windows × amd64/arm64)
...
# 启动 Dashboard
./dashboard -addr 0.0.0.0:8553 \
  -secret <Agent密钥> -web-token <Web口令> \
  -tls-cert cert.pem -tls-key key.pem \
  -db data/probe.db

# 启动 Agent
./agent -server 你的域名:8553 -token <Agent密钥> -name "web-01" -tls
```

二进制是 `CGO_ENABLED=0` 静态编译,无外部依赖,丢到目标机器直接跑。

### 五、常见操作

**登录面板**:浏览器打开访问地址(口令已自动填在 URL 里),或单独记口令到登录页手输。Session 有效期 7 天。

**修改主机名**:面板详情页点主机名旁的 ✎ 图标即可改名;或调 API:

```bash
curl -X POST -H "Content-Type: application/json" \
  -d '{"name":"生产服务器-01"}' \
  http://你的域名:8553/api/servers/<agentID>/rename
```

改名持久生效(存 SQLite),Agent 重新上报不会覆盖。
...
| Dashboard | 说明 |
|---|---|
| `DOMAIN` | 域名(自动申请 Let's Encrypt) |
| `CERT` / `KEY` | 已有证书路径(优先级最高,与 `DOMAIN` 二选一) |
| `SECRET` | Agent 密钥(留空自动生成) |
| `WEB_TOKEN` | Web 口令(留空自动生成) |
| `PORT` | 监听端口,默认 `8553` |

| Agent | 说明 |
|---|---|
| `SERVER` | Dashboard 地址(**必填**) |
| `TOKEN` | Agent 密钥(**必填**) |
| `NAME` | 主机名,默认取系统主机名 |
| `TLS=1` | 开启 TLS |
| `INSECURE=1` | 自签证书时跳过校验 |
| `INTERVAL` | 采集间隔,默认 `3s` |
...
这样 Agent 上报的 secret 和所有指标数据在网络上是加密的,无法被中间人窃听或篡改。
...
- **登录鉴权**:`-web-token` 配置后,Web UI / API / WS 都需要登录(密码用恒定时间比较,session 存内存,cookie 为 `HttpOnly`+`SameSite=Strict`+`Secure`)。
- **Agent 认证**:`-secret` 用恒定时间比较(`subtle.ConstantTimeCompare`),认证失败不返回具体原因,直接关闭连接。
- **认证超时**:未认证的 Agent 连接必须在 10 秒内完成握手。
- **一次性 token 换 session**:`?token=` 访问后立即换取 session cookie 并 302 重定向到不带 token 的 URL,避免 token 残留在浏览器历史、地址栏或 Referer 中。WebSocket 和 API 后续均用 cookie 鉴权。
- **凭据不落 unit 文件**:一键安装脚本将密钥写入权限 `600` 的 `EnvironmentFile`,不写进默认全机可读的 systemd unit 文件,也不出现在 `/proc/<pid>/cmdline`。
- **默认只监听本机**:`-addr` 默认 `127.0.0.1:8000`。
- **XSS 防护**:前端表格用 DOM API(`textContent`)渲染,不拼 `innerHTML`。
- **响应安全头**:`X-Content-Type-Options: nosniff`、`X-Frame-Options: DENY`、`Referrer-Policy: no-referrer`。
- **CSP 收紧**:Chart.js 改为本地自托管,CSP 的 `script-src`/`connect-src` 仅允许同源,不再依赖外部 CDN。
- **历史查询上限**:限制在 24 小时内。

**生产部署建议**:Dashboard 放在 Nginx/Caddy + TLS 后面,用 `-addr 127.0.0.1:8000`,通过反向代理做 TLS 终止;`-secret` 和 `-web-token` 都用足够长的随机串。
...
scripts/            部署脚本(build-all / install-dashboard / install-agent)
```
